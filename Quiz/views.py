...
@login_required
def quiz(request):
    question=questions.objects.filter().order_by('?').first()
    lastattempt=None
    futuretime=None
    hourslimit=24
...
def submit_answer(request,quest_id):
    if request.method=='POST':
        ...
        question=questions.objects.filter().order_by('?').exclude(id=quest_id).first()
        
        if 'skip' in request.POST:
            if question and models.UserSubmittedAnswer.objects.count()<29:
